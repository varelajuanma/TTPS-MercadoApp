    ...
    <div class="col-md-8">
      <div class="card">
        ...
        <div class="card-body">
          ...
          <u><strong>Precio:</strong></u> ${{$publicacion->precioActualFormateado()}} <br>

        </div>
      </div>
      <br>
      <form form action="{{route('agregaralcarrito') }}" method="POST">
        @csrf
        <button type="submit" class="btn btn-primary">Agregar al carrito</button>
        <input type="hidden" name="publicacion" value="{{$publicacion->id}}" >
        <input type="number" name="cantidad" value=1 min="1" max="{{$publicacion->stock}}"> Unidad/es
        @error('cantidad')
